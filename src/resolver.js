...
function resolveVariables(text, context, options = {}) {
    ...
    return text.replace(PLACEHOLDER_REGEX, (match, path) => {
      ...
      if (value === undefined) {
        switch (mergedOptions.onMissing) {
          ...
          case 'throw': throw new MissingVariableError(trimmedPath);
          case 'placeholder': return match;
          default:
            if (typeof mergedOptions.onMissing === 'function') {
              ...
              try {
                ...
                console.error(`@tickit/variable-resolver: Error in custom onMissing handler for path '${trimmedPath}':`, funcError);
                throw new ResolveError(`Resolution failed: Error in custom onMissing handler - ${funcError.message}`); // Wrap custom handler error
              }
            }
      ...
      if (value === null) {
        switch (mergedOptions.onNull) {
          case 'emptyString': return '';
          case 'placeholder': return match;
          case 'throw': throw new NullVariableError(trimmedPath);
          case 'null': return 'null'; // Default
          default:
            if (typeof mergedOptions.onNull === 'function') {
              ...
              try {
                ...
                console.error(`@tickit/variable-resolver: Error in custom onNull handler for path '${trimmedPath}':`, funcError);
                throw new ResolveError(`Resolution failed: Error in custom onNull handler - ${funcError.message}`); // Wrap custom handler error
              }
            }
  ...
  } catch (error) { // <--- Ensure this variable is named 'error'
    // Re-throw known resolver errors
    if (error instanceof ResolveError) { // <--- Check against the variable 'error'
      throw error;
    }
